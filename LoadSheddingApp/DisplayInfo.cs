...
namespace LoadSheddingApp
{
    public class DisplayInfo
    {
        ...
        public void Result()
        {
            ...
            {
                Console.WriteLine($"Your area is currently not experiencing any loadshedding, however there is loadshedding scheduled at {_start} until {_end}");
            }
            else if ((current > start) && (_start == emptyTime))
            {
                Console.WriteLine($"Your area is currently not experiencing any loadshedding, however there is loadshedding scheduled at {start} until {end}");
            }
            else if ((current > end) && (current < _start) && (_end == afterMidNight))
            {
                Console.WriteLine($"Your area is currently not experiencing loadshedding, however there is loadshedding scheduled at {_start} until {_end}");
            }
            ...
            {
                Console.WriteLine($"Your area is currently not experiencing loadshedding and there are no loashedding for scheduled tonight.");
            }
            else if ((current > _end) && (current < start))
            {
                Console.WriteLine($"Your area is currently not experiencing loadshedding, however there is loadshedding scheduled at {start} until {end}");
            }
            ...
            {
                Console.WriteLine($"Your area is currently not experiencing loadshedding and there is no loashedding scheduled for tonight.");
            }

            // During loadshedding hours
            else if ((current <= _start && current >= _end) && (start == emptyTime))
            {
